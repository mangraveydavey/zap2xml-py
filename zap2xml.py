...
def get_args():
    ...
    parser.add_argument(
        ...
        help='Raw zap2it input parameter.  (?)')
    parser.add_argument(
        '--is-override', dest='zap_isOverride', type=str, default='true',  # Lowercase 'true'
        help='Raw zap2it input parameter.  (?)')
    parser.add_argument(
        '--language', dest='zap_languagecode', type=str, default='en-us',  # Updated to 'en-us'
        help='Raw zap2it input parameter.  (Language.)')
    parser.add_argument(
        '--pref', dest='zap_pref', type=str, default='16,128',  # Updated pref
        help='Raw zap2it input parameter.  (Preferences?)')
    parser.add_argument(
        '--timespan', dest='zap_timespan', type=int, default=3,  # Keep as 3, or change to 2 if needed
...
def get_cached(cache_dir, cache_key, delay, url):
    ...
    if cache_path.is_file():
        ...
        try:
            ...
            result = resp.read()
            # Debug: Print the raw response to check its content
            print('Response content:', result.decode('utf-8', errors='ignore')[:500])  # Print first 500 chars
            if not result.strip():  # Check if response is empty
                print('Empty response received, treating as 400 error.')
                result = (
                    b'{'
                    b'"note": "Empty response, treating as 400 error, skipping.",'
                    b'"channels": []'
                    b'}')
...
def main():
    ...
    for i in range(int(7 * 24 / args.zap_timespan)):
        ...
        qs['time'] = i_time
        url = 'https://tvlistings.gracenote.com/api/grid?' + urllib.parse.urlencode(qs)  # Use /api/grid and build query params without duplicates
        ...
        try:
            ...
            print('Response was not valid JSON, skipping this time slot.')
            d = {'note': 'Invalid JSON response, skipping.', 'channels': []}
